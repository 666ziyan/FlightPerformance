...
from_NYC_2019 <- US_2019_cleaning %>%
  filter((ORIGIN=='JFK') | (ORIGIN=='LGA') | (ORIGIN=='EWR')) %>%
  arrange(CRS_DEP_TIME) %>%
  group_by(dep_gr=cut(CRS_DEP_TIME, breaks=seq(0, 2400, by=300))) %>% 
  summarise_at(vars(DEP_DELAY, ARR_DELAY), list(Mean=mean))
levels(from_NYC_2019$dep_gr) = time_lv

from_NYC_2020 <- US_2020_cleaning %>%
  filter((ORIGIN=='JFK') | (ORIGIN=='LGA') | (ORIGIN=='EWR')) %>%
  arrange(CRS_DEP_TIME) %>%
  group_by(dep_gr=cut(CRS_DEP_TIME, breaks=seq(0, 2400, by=300))) %>% 
  summarise_at(vars(DEP_DELAY, ARR_DELAY), list(Mean=mean))
levels(from_NYC_2020$dep_gr) = time_lv

ggplot(from_NYC_2019, aes(x=dep_gr)) + 
  ...
  labs(x="CRS Departure Time", y="Average Delay (min)", title='Flight Performance from NYC in 2019')



ggplot(from_NYC_2020, aes(x=dep_gr)) + 
  geom_point(aes(y=DEP_DELAY_Mean, color='Departure')) + 
  geom_point(aes(y=ARR_DELAY_Mean, color='Arrival')) +
  scale_colour_manual("", breaks=c('Departure','Arrival'), values=c('red','blue')) +
  labs(x="CRS Departure Time", y="Average Delay (min)", title='Flight Performance from NYC in 2020')
