...
levels(from_NY_2019_allu$dep_gr) = time_lv_index
...
from_NY_2020_allu <- NY_2020_cleaning %>%
  ...
  summarise_at(vars(DEP_DELAY, ARR_DELAY), list(Mean=mean)) %>%
  group_by(mon = str_sub(FL_DATE,6,7))
levels(from_NY_2020_allu$dep_gr) = time_lv_index
...
from_NY_allu <- rbind(from_NY_2019_allu, from_NY_2020_allu)

from_NY_2019_allu_01 <- from_NY_2019_allu %>%
  filter(mon == "01")
ggplot(from_NY_2019_allu_01, aes(x=DATE)) + 
  geom_line(aes(y = DEP_DELAY_Mean, color = dep_gr)) + 
  geom_hline(yintercept = 0, color = "#116530") + 
  labs(x="CRS Departure DATE", y="Average Delay (min)", title='Daily Early / Late Flight Performance from NY') +
  facet_wrap(~mon, ncol=4)
```
