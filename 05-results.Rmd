...
for (i in 1:5) {
  ...
  p1 <- ggplot(data = delayDetailCount, mapping = aes(x = CARRIER_AIRLINE_ID, y = count)) +
    geom_bar(stat = 'identity', fill = '#FFAFAF', color = '#5584AC') +
    labs(x = "CARRIER AIRLINE ID", y = 'DELAY COUNT')
  ...
  p2 <- ggplot(data = delayDetailAverage, mapping = aes(x = CARRIER_AIRLINE_ID, y = delayDetailMean)) +
...
from_NY_2019 <- NY_2019_cleaning %>%
  filter(((ORIGIN_CITY_NAME=='New York, NY') | (ORIGIN=='EWR'))) %>% 
  arrange(CRS_DEP_TIME) %>%
  group_by(dep_gr=cut(CRS_DEP_TIME, breaks=seq(0, 2400, by=300))) %>% 
  # ungroup() %>%
  # mutate(dep_delay_mean = mean(DEP_DELAY, na.rm=TRUE)) %>%
  summarise_at(vars(DEP_DELAY, ARR_DELAY), list(Mean=mean))
levels(from_NY_2019$dep_gr) = time_lv

ggplot(from_NY_2019, aes(x=dep_gr)) + 
  geom_point(aes(y=DEP_DELAY_Mean, color='Departure Delay')) + 
  geom_point(aes(y=ARR_DELAY_Mean, color='Arrival Delay')) +
  scale_colour_manual("", breaks=c('Departure Delay','Arrival Delay'), values=c('red','blue')) +
